d = read.csv('/Users/mazim/Documents/RahmeLab/Analysis/KerV/DataFrames/KerV start_stop1.csv')
...
getPositionC <- function(tregionC, d1, d2, n) {
  
  name = paste0('tregionC_mod', n)
  assign(name, tregionC, envir = parent.frame())
  dfC = get(name)
  ...
  all_df_C = data.frame(start = c(d1$start, d2$start), 
                      end = c(d1$end, d2$end),
                      mthyl = c(rep('CCGG', nrow(d1)),
                                rep('GGCC', nrow(d2))))
  ...
  for(i in 1:nrow(dfC)){
    x1 = which(dplyr::between(d1$start, dfC$searchStart[i], dfC$searchEnd[i]))
    x2 = which(dplyr::between(d2$start, dfC$searchStart[i], dfC$searchEnd[i])) + nrow(d1)
...
# tregionC
# write.csv(tregionC, '/Users/mazim/Documents/RahmeLab/Analysis/KerV/DataFrames/PromoterPosition/tregionC.csv')
...
getPositionA <- function(tregionA, d3, d4, d5, d6, n) {
  
  name = paste0('tregionA_mod', n)
  assign(name, tregionA, envir = parent.frame())
  dfA = get(name)
  ...
  all_df_A = data.frame(start = c(d3$start, d4$start, d5$start, d6$start), 
                        end = c(d3$end, d4$end, d5$end, d6$end),
                        mthyl = c(rep('GATC', nrow(d3)),
                                  rep('CTAG', nrow(d4)),
                                  rep('TCGA', nrow(d5)),
                                  rep('AGCT', nrow(d6))))
  ...
  for(i in 1:nrow(tregion)){
    y1 = which(dplyr::between(d1$start, dfA$searchStart[i], dfA$searchEnd[i]))
    y2 = which(dplyr::between(d2$start, dfA$searchStart[i], dfA$searchEnd[i])) + nrow(d1)
    y3 = which(dplyr::between(d3$start, dfA$searchStart[i], dfA$searchEnd[i])) + nrow(d1) + nrow(d2)
    y4 = which(dplyr::between(d4$start, dfA$searchStart[i], dfA$searchEnd[i])) + nrow(d1) + nrow(d2) + nrow(d3)
...
tregion300A <- getPositionA(tregionA, d3, d4, d5, d6, 0)
tregion300C <- getPositionC(tregionC, d1, d2, 0)
tregionA_mod1000 <- getPositionA(tregionA, d3, d4, d5, d6, 1000)
tregionC_mod100 <- getPositionC(tregionC, d1, d2, 100)
# write.csv(tregionA, '/Users/mazim/Documents/RahmeLab/Analysis/KerV/DataFrames/PromoterPosition/tregionA.csv')
